...
import './unpaidOrdersUser.scss';
import { useNavigate } from 'react-router-dom';
import { FaRegSadTear } from 'react-icons/fa'; 

const UnpaidOrdersUser = () => {
                ...
                <div className="unpaid-orders__box">
                    {items.length === 0 ? (
                        <div className="unpaid-orders__empty">
                            ...
                            <h3>Hozircha to'lov qilinmagan mahsulotlar yo'q</h3>
                            <p>Siz hali hech qanday mahsulot tanlamagansiz.</p>
                            <a href="" className="unpaid-orders__link">Mahsulotlar bo'limiga qaytish</a>
                        </div>
                    ) : (
                            ...
                            {items.map(item => (
                                <div key={item.id} className="unpaid-orders__product-box order-user__window-box" onClick={() => navigate(`/product/${item.id}`)} style={{ cursor: "pointer" }}>
                                    ...
                                    <div className="unpaid-orders__product__price-add">
                                        <p className="unpaid-orders__product__price">{item.price} so'm</p>
                                        <p className="unpaid-orders__totalPrice">{item.price * item.count} so'm</p>
                                    </div>
                                </div>
                            ))}
                        </div>
...
export default UnpaidOrdersUser;
